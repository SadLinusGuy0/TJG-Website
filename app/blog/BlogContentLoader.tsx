...
export default function BlogContentLoader() {
  ...
  const selectedUnit = searchParams?.get('unit') || undefined;
  ...
  useEffect(() => {
    async function loadBlogData() {
      try {
        ...
        // Determine category to filter by based on selected unit
        const categoryId = selectedUnit
          ? sortedCategories.find(c => c.slug === selectedUnit)?.id
          : undefined;

        // Fetch posts
        const fetchedPosts = await fetchPosts({ perPage: 20, categoryId });
  ...
  // Custom order for categories (for display)
  const customOrder = ['blender', 'unit-1', 'unit-2', 'unit-3', 'unit-4', 'unit-5', 'unit-6', 'unit-7', 'final-major-project', 'uncategorized'];
          ...
          <div className="main-content" style={{ paddingTop: 0 }}>
            <div className="blank-div">
              <div className="container1">
                <div className="title">Error Loading Blog</div>
              </div>
