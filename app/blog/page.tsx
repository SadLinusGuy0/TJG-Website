...
import { fetchPosts, fetchCategories, getFeaturedImageUrlAsync, WPPost, WPCategory } from "../../lib/wordpress";
...
export default async function BlogIndex({ searchParams }: Props) {
  ...
  // Determine category to filter by based on selected unit (category slug)
  const categoryId = selectedUnit
    ? categories.find(c => c.slug === selectedUnit)?.id
    : undefined;

  try {
    posts = await fetchPosts({ perPage: 20, categoryId });
    console.log('Successfully fetched posts:', posts.length);
  } catch (error) {
    console.error('Failed to fetch posts:', error);
    console.error('This might be due to missing WP_API_URL environment variable');
  }
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          ...
          @media (min-width: 768px) and (max-width: 1023px) {
            ...
            /* tablet-specific adjustments here if needed */
          }
            ...
            <div className="blank-div">
              <div className="theme-container">
                {await Promise.all(posts.map(async (post) => {
                  const featuredImageUrl = await getFeaturedImageUrlAsync(post);
                  
                  // Temporary test - uncomment to test thumbnail display
                  // const testImageUrl = 'https://via.placeholder.com/120x80/3B82F6/FFFFFF?text=Test';
                  // const featuredImageUrl = testImageUrl;
                  ...
                  console.log('Post in index:', {
                    ...
                    hasEmbedded: !!post._embedded?.['wp:featuredmedia'],
                    embeddedMedia: post._embedded?.['wp:featuredmedia'],
                    featuredImageUrl,
                    willShowImage: !!featuredImageUrl
                  });
                            ...
                            <div style={{ 
                              ...
                              {post.categories && post.categories.length > 0 && (
                                <div style={{
                                  ...
                                  {post.categories.map((catId: number) => categoryMap.get(catId)).filter(Boolean).join(', ')}
                                </div>
                              )}
                            </div>
                          </div>
